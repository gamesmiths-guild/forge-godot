...
/// </summary>
/// <remarks>
/// <para>This is Raycast 2D held open. The ray inputs are re-resolved on every cast, so binding the origin and
...
[StatescriptCollisionMaskInputs(CollisionLayerSpace.Physics2D, RaycastNodeParameters2D.MaskInput)]
public class Ray2DNode(
	bool collideWithAreas = false,
	bool hitFromInside = false,
	bool oneShot = false) : StateNode<Ray2DNodeContext>
{
	/// <summary>
	...
	{
		graphContext.GetNodeContext<Ray2DNodeContext>(NodeID).LastHit = null;
		Cast(graphContext);
	}
	...
	{
		Ray2DNodeContext nodeContext =
			graphContext.GetNodeContext<Ray2DNodeContext>(NodeID);
	...
	private void Cast(GraphContext graphContext)
	{
		...
		PhysicsDebugDraw2D.SetLine(nodeContext.DebugMarker, segment.From, segment.To);
		...
		PhysicsDebugDraw2D.FlashTarget(graphContext, result.Entity, PhysicsDebugDraw2D.RayHitColor, _collideWithAreas);
