...
/// <summary>
/// Resolves the scale of the 2D node an entity lives on.
/// </summary>
...
internal sealed class EntityScale2DResolver(IEntityResolver entityResolver, string nodePath, TransformSpace space)
	: SpatialResolverBase2D(entityResolver, nodePath)
{
	...
	{
		Vector2 scale = _space == TransformSpace.Local ? spatialNode.Scale : spatialNode.GlobalScale;
		return new Variant128(new NumericsVector2(scale.X, scale.Y));
	}
}
