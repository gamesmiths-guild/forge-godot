	...
	string nodePath = "") : StateNode<MoveTo2DNodeContext>
{
	...
	protected override void OnActivate(GraphContext graphContext)
	{
		...
		{
			GD.PushWarning(
				"Statescript: Move To 2D found no Node2D for its entity" +
		...
		{
			GD.PushWarning("Statescript: Move To 2D could not resolve a destination. The move was skipped.");
			return;
		}
		...
		nodeContext.Destination = new Vector2(destination.X, destination.Y);
	...
	private static double ResolveDurationFromSpeed(MoveTo2DNodeContext nodeContext, double speed)
	{
		if (speed <= 0)
		{
			return 0;
	...
	private void ApplyPosition(Node2D spatialNode, MoveTo2DNodeContext nodeContext, double progress)
	{
		float clamped = Mathf.Clamp((float)progress, 0.0f, 1.0f);
		float eased = Ease(_easing, clamped);

		Vector2 position = nodeContext.StartPosition.Lerp(nodeContext.Destination, eased);
		...
		// destination and break the arrival contract. Subtracted rather than added because 2D screen up is -Y, so a
		// positive arc height arcs the way it reads.
