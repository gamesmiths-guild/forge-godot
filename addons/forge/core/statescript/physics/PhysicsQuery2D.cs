...
/// <summary>
/// The physics queries the 2D Statescript nodes and resolvers run, in one place so a ray cast from a Condition node and
/// a ray cast from a line-of-sight resolver answer the same question the same way.
/// </summary>
internal static class PhysicsQuery2D
{
	...
	public static World2D? ResolveWorld(GraphContext graphContext)
	{
		return TryResolveContextNode(graphContext, out Node2D? ownerNode) ? ownerNode.GetWorld2D() : null;
	}
	...
	public static bool TryResolveContextNode(
		...
		[NotNullWhen(true)] out Node2D? contextNode)
	{
		if (graphContext.TryGetActivationContext(out AbilityBehaviorContext? abilityContext))
		{
			return ForgeEntityBridge.TryGetSpatialNode2D(abilityContext.Owner, out contextNode);
		}
	...
	public static bool IsCastable(Vector2 direction, float maxDistance)
	{
		...
		out RaycastResult2D result)
	{
		...
		Vector2 target = origin + (direction.Normalized() * maxDistance);
		...
		Vector2 position = hit["position"].AsVector2();
		Node2D collider = hit["collider"].As<Node2D>();
	...
	/// begins at a character's own origin starts inside that character's own collider, and <c>hitFromInside</c> can
	/// only say whether shapes like that count at all - it cannot keep the caster's own body out while leaving the wall
	/// it is standing against in. The far end has the same problem in reverse - a line drawn to a marker inside someone
	/// is stopped by the body that marker belongs to - which is why this takes a list rather than one entity.</para>
		...
		ISet<IForgeEntity> into)
	{
		...
		foreach (Area2D overlapping in area.GetOverlappingAreas())
		{
