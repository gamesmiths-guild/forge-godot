...
internal static class PhysicsQuery3D
{
	...
	/// </summary>
	/// <remarks>
	/// The one space-state query with no shape behind it, and not the same thing as an overlap with a tiny sphere: a
	/// point is exact and needs no radius invented for it. What is standing exactly here, and — in 2D, where the
	/// cursor already names a world point — what the player just clicked on.
	/// </remarks>
	/// <param name="world">The world to query.</param>
	...
	/// </summary>
	/// <remarks>
	/// <para>The check the non-solving movement nodes create a need for. Set Position 3D and Move To 3D drive the
	...
	public static bool CanFit(PhysicsBody3D body, Transform3D from, Vector3 motion)
	{
	...
	private static RaycastResult3D ReadResult(GodotDictionary rest, float distance)
	{
		var collider = GodotObject.InstanceFromId((ulong)rest["collider_id"]) as Node3D;

		return new RaycastResult3D(
			rest["point"].AsVector3(),
			rest["normal"].AsVector3(),
			collider,
			ForgeEntityBridge.TryGetEntityInHierarchy(collider, out IForgeEntity? entity) ? entity : null,
			distance);
	}
