...
If any node in the graph uses an `AbilityActivationDataResolverResource`, the behavior automatically detects the associated `IActivationDataProvider`, builds the matching `GraphAbilityBehavior<TData>`, and lets the resolver read the selected activation-data member directly from the typed payload.
