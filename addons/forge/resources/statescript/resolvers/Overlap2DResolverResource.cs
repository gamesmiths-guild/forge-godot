...
[Tool]
[GlobalClass]
public partial class Overlap2DResolverResource : StatescriptResolverResource
{
	/// <inheritdoc/>
	public override string ResolverTypeId => "Overlap2D";
	...
	/// editor always authors one, starting on an Entity Position 2D resolver so a fresh query is centred on the
	/// caster.
	/// </remarks>
	[Export]
	...
	/// <summary>
	/// Gets or sets the nested resolver providing how the shape is turned, in radians. Unset leaves it unturned.
	/// </summary>
	[Export]
	...
	public override void BindInput(Graph graph, ForgeNode runtimeNode, string nodeId, byte index)
	{
		...
		var propertyName = new StringKey($"__overlap2d_{nodeId}_{index}");
		...
		out IObjectArrayResolver? objectArrayResolver)
	{
		...
		IPropertyResolver? rotationResolver = Rotation is null
			? null
			: AdaptResolverForExpectedType(Rotation.BuildResolver(graph), typeof(double));

		objectArrayResolver = new Overlap2DResolver(
	...
	private IObjectResolver<Shape2D> BuildShapeResolver(Graph graph)
	{
		// An unset shape resolves to nothing rather than to a default circle: a query with no shape authored should
		...
		if (Shape is null
			...
			|| objectResolver is not IObjectResolver<Shape2D> shapeResolver)
		{
			return new ShapePicker2DResolver(null);
		}
