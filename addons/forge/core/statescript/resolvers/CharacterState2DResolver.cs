...
/// </summary>
/// <remarks>
/// <para>This exists because these are <em>methods</em> on <see cref="CharacterBody2D"/> rather than properties, and
...
/// <see cref="CharacterBody2D.MoveAndSlide"/>, so a body the game has not moved this frame reports what it last found
/// — which is the same answer the game's own code would get, and the right one.</para>
/// <para>Only a <see cref="CharacterBody2D"/> has them. A rigid body is not moved by slides and has no notion of
...
internal sealed class CharacterState2DResolver(
	IEntityResolver entityResolver,
	string nodePath,
	CharacterStateQuery state) : SpatialResolverBase2D(entityResolver, nodePath)
{
	...
	protected override Variant128 ResolveFrom(Node2D spatialNode, GraphContext graphContext)
	{
		if (spatialNode is not CharacterBody2D characterBody)
		{
			ReportUnusableNodeOnce(
				...
				" CharacterBody2D does. Resolving to false.");
