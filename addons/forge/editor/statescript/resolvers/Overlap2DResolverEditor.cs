...
/// </summary>
[Tool]
internal sealed partial class Overlap2DResolverEditor : NodeEditorProperty
{
	private static readonly Type[] _shapeExpectedTypes = [typeof(Shape2D)];
	private static readonly Type[] _positionExpectedTypes = [typeof(NumericsVector2)];
	...
	public override string DisplayName => "Overlap 2D";
	...
	public override void Setup(
		...
		bool isArray)
	{
		var resource = property?.Resolver as Overlap2DResolverResource;
		...
		_includeAreasCheckBox = new CheckBox
		{
			...
			TooltipText = "Count areas as overlaps as well as bodies, for entities whose hurtbox is an Area2D.",
		};
	...
	{
		property.Resolver = new Overlap2DResolverResource
		{
	...
	private void BuildOperandPickers(
		StatescriptGraph graph,
		Overlap2DResolverResource? resource,
		VBoxContainer root)
	{
		// Seeded with a circle, an Entity Position 2D and the caster: a nested operand has no unbound state, so an
		// unseeded shape would be nothing, an unseeded position would be the world origin, and an unseeded ignore would
		// let a blast catch the caster who cast it. The rotation is deliberately not seeded - most queries are
		// unturned, and an unset one is read as no rotation at all.
