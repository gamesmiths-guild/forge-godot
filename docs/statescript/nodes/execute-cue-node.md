...
- `Magnitude` (int), `Normalized Magnitude` (float), and `Source` (entity) are optional. Each starts at **(None)** and offers it in its resolver dropdown; when they and `Custom Parameters` are all on (None) the cues are executed with no parameters at all, otherwise the resolved values are bundled into the cue's `CueParameters`. See [The `(None)` Input Option](../README.md#the-none-input-option).
