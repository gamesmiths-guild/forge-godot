...
/// <summary>
/// Resolver resource that reads the entity that applied an active effect — its ownership source — from an
/// <see cref="ActiveEffectHandle"/> produced by a nested resolver.
/// </summary>
[Tool]
[GlobalClass]
public partial class ActiveEffectSourceResolverResource : EntityResolverResourceBase
{
	/// <inheritdoc/>
	...
	public override IEntityResolver BuildEntityResolver(Graph graph)
	{
		...
		{
			GD.PushError(
				"Statescript: Active Effect Source resolver requires an Active Effect source. Falling back to the " +
				"ability owner.");
			return new AbilityOwnerResolver();
		}

		return new ActiveEffectSourceResolver(typedHandleResolver);
	}
}
