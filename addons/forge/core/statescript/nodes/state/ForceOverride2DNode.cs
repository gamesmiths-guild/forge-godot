...
/// the same guarantee Collision Override 2D gives: a cancelled ability cannot leave a body accelerating forever.</para>
...
public class ForceOverride2DNode(string nodePath = "") : StateNode<ForceOverride2DNodeContext>
{
	/// <summary>
	...
	protected override void DefineParameters(List<InputProperty> inputProperties, List<OutputVariable> outputVariables)
	{
		...
		inputProperties.Add(new InputProperty("Force", typeof(NumericsVector2), IsOptional: true));
		inputProperties.Add(new InputProperty("Torque", typeof(double), IsOptional: true));
	}
	...
	protected override void OnActivate(GraphContext graphContext)
	{
		...
		if (!ForgeEntityBridge.TryGetSpatialNode2D(entity, _nodePath, out Node2D? spatialNode)
			|| spatialNode is not RigidBody2D rigidBody)
		{
		...
		if (graphContext.TryResolve(InputProperties[ForceInput].BoundName, out NumericsVector2 force))
		{
			nodeContext.PreviousForce = rigidBody.ConstantForce;
			nodeContext.WroteForce = true;
			rigidBody.ConstantForce = new Vector2(force.X, force.Y);
		}
		...
		if (graphContext.TryResolve(InputProperties[TorqueInput].BoundName, out double torque))
		{
			...
			rigidBody.ConstantTorque = (float)torque;
		}
	...
	protected override void OnUpdate(double deltaTime, GraphContext graphContext)
	{
		...
		if (nodeContext.WroteForce
			&& graphContext.TryResolve(InputProperties[ForceInput].BoundName, out NumericsVector2 force))
		{
			body.ConstantForce = new Vector2(force.X, force.Y);
		}

		if (nodeContext.WroteTorque
			&& graphContext.TryResolve(InputProperties[TorqueInput].BoundName, out double torque))
		{
			body.ConstantTorque = (float)torque;
		}
		...
		PhysicsDebugDraw2D.SetArrow(nodeContext.DebugMarker, body.GlobalPosition, body.ConstantForce);
	}
	...
	private void ReportMissingBodyOnce()
	{
		...
		GD.PushWarning(
			"Statescript: Force Override 2D found no RigidBody2D for its entity" +
