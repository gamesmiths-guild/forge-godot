...
[Tool]
[GlobalClass]
public partial class NavPathLength2DResolverResource : StatescriptResolverResource
{
	/// <inheritdoc/>
	...
	public override void BindInput(Graph graph, ForgeNode runtimeNode, string nodeId, byte index)
	{
		DefineAndBindInputProperty(
			graph,
			runtimeNode,
			$"__navpathlength2d_{nodeId}_{index}",
			index,
			BuildResolver(graph));
	}
	...
	{
		return new NavPathLength2DResolver(BuildPoint(From, graph), BuildPoint(To, graph));
	}
