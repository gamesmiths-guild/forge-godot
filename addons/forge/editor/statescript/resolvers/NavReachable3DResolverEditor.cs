...
/// </summary>
[Tool]
internal sealed partial class NavReachable3DResolverEditor : NodeEditorProperty
{
	...
	public override string DisplayName => "Nav Reachable 3D";
	...
	public override void Setup(
		...
		bool isArray)
	{
		var resource = property?.Resolver as NavReachable3DResolverResource;
		...
		// Seeded with Entity Position 3D on both ends, matching Line Of Sight 3D: the two points are almost always
		...
		_tolerancePicker.Initialize(
			graph,
			resource?.Tolerance ?? new VariantResolverResource { Value = NavReachable3DResolver.Tolerance },
			"Within:",
	...
	{
		property.Resolver = new NavReachable3DResolverResource
		{
		...
		bool folded)
	{
		...
		picker.Initialize(
			graph,
			existing ?? new EntityPosition3DResolverResource(),
			title,
			_pointExpectedTypes,
			isArray: false,
			folded,
