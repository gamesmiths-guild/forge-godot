...
internal sealed partial class ArrayVariableResolverEditor : NodeEditorProperty
{
	...
	private StatescriptVariableType _selectedValueType = StatescriptVariableType.Int;
	...
	public override string DisplayName => "Array Variable";
		...
		bool isArray)
	{
		...
		if (property?.Resolver is ArrayVariableResolverResource existing)
		{
			...
			_selectedValueType = existing.ValueType;
		}
	...
	private void OnGraphVariableChanged(StatescriptGraph graph, long index)
	{
		...
		if (!string.IsNullOrEmpty(_selectedVariableName))
		{
			...
			{
				if (variable.VariableName == _selectedVariableName)
				{
					_selectedValueType = variable.VariableType;
					break;
				}
			}
		}
	...
	private void PopulateGraphVariableDropdown(StatescriptGraph graph)
	{
		...
		foreach (StatescriptGraphVariable variable in graph.Variables)
		{
			if (!variable.IsArray
				...
				|| !StatescriptVariableTypeConverter.IsCompatible(_expectedType, variable.VariableType))
			{
				continue;
			}
			...
			if (set is not null)
			{
				foreach (ForgeSharedVariableDefinition definition in set.Variables)
				{
					if (!definition.IsArray
						...
						|| !StatescriptVariableTypeConverter.IsCompatible(_expectedType, definition.VariableType))
					{
						continue;
					}
	...
	{
		_selectedValueType = StatescriptVariableType.Int;
		...
		foreach (ForgeSharedVariableDefinition definition in set.Variables)
		{
			if (definition.VariableName == _selectedVariableName)
			{
				_selectedValueType = definition.VariableType;
				return;
			}
		}
	}
