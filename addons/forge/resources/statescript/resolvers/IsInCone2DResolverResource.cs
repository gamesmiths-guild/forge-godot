...
[Tool]
[GlobalClass]
public partial class IsInCone2DResolverResource : ValueResolverResourceBase
{
	/// <summary>
	...
	/// <inheritdoc/>
	protected override string PropertyNamePrefix => "isincone2d";
	...
	protected override IPropertyResolver CreateResolver(Graph graph)
	{
		...
		IPropertyResolver pointResolver = Point is null
			? new EntityPosition2DResolver(new AbilityTargetResolver(), string.Empty, TransformSpace.Global)
			: AdaptResolverForExpectedType(Point.BuildResolver(graph), typeof(NumericsVector2));
