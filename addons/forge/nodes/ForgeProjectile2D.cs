...
/// <see cref="CollisionShape2D"/> child becomes the query's shape — a projectile built from several shapes, or from a
/// <see cref="CollisionPolygon2D"/>, has to turn this off and keep <see cref="Speed"/> under the thinnest collider it
...
public partial class ForgeProjectile2D : Area2D, IInstantiationReceiver
{
	...
	private CollisionShape2D? _sweptShape;
	...
	private void ResolveSweptShape()
	{
		...
		{
			if (child is not CollisionShape2D { Disabled: false, Shape: not null } collisionShape)
			{
				continue;
			}
		...
		{
			GD.PushWarning(
				$"ForgeProjectile2D [{Name}] is set to sweep but has no enabled CollisionShape2D child to sweep " +
		...
		{
			GD.PushWarning(
				$"ForgeProjectile2D [{Name}] sweeps one shape and has {enabledShapes} enabled. Only " +
	...
	private void SweepStep(Vector2 motion)
	{
		...
		Transform2D start = _sweptShape.GlobalTransform;
		float traveled = GlobalPosition.DistanceTo(_launchPosition);

		while (PhysicsQuery2D.TryShapecast(
			world,
			_sweptShape.Shape,
			start,
			motion,
			...
			out RaycastResult2D result))
		{
			...
			// CollisionObject2D - a TileMapLayer owns its own - and one that cannot be excluded would stop every
