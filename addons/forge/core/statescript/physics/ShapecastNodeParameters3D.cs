...
/// with a volume instead of a line, so a graph swapped from one to the other keeps every binding it had.
/// </remarks>
internal static class ShapecastNodeParameters3D
{
	/// <summary>
	...
	/// <summary>
	/// Input property index for how the shape is turned.
	/// </summary>
	...
	{
		inputProperties.Add(new InputProperty("Shape", typeof(Shape3D)));
		inputProperties.Add(new InputProperty("Origin", typeof(NumericsVector3)));
		inputProperties.Add(new InputProperty("Direction", typeof(NumericsVector3)));
		...
		outputVariables.Add(new OutputVariable("Hit Position", typeof(NumericsVector3)));
		outputVariables.Add(new OutputVariable("Hit Normal", typeof(NumericsVector3)));
	...
	{
		result = default;
		hitTransform = Transform3D.Identity;
		segment = default;
		shape = null;
		...
		if (world is null
			...
			|| !graphContext.TryResolve(inputProperties[OriginInput].BoundName, out NumericsVector3 origin)
			|| !graphContext.TryResolve(inputProperties[DirectionInput].BoundName, out NumericsVector3 direction))
		{
			return false;
		}
		...
		hitTransform = new Transform3D(ResolveBasis(rotation), from);

		if (!PhysicsQuery3D.IsCastable(along, (float)maxDistance))
		{
			return false;
		}

		Vector3 motion = along.Normalized() * (float)maxDistance;
		...
		segment = new RaySegment3D(from, from + motion);

		return PhysicsQuery3D.TryShapecast(
			world,
			shape,
			hitTransform,
			motion,
	...
	public static void WriteOutputs(
		...
		in RaycastResult3D result)
	{
		RaycastNodeParameters3D.WriteOutputs(graphContext, outputVariables, result);
	}

	// A zero quaternion is what an unfilled rotation operand resolves to, and Godot rejects one outright rather than
	// treating it as no rotation.
	private static Basis ResolveBasis(NumericsQuaternion rotation)
	{
