...
/// Collision Override 2D, which puts the original bits back even when the ability is interrupted.
/// </remarks>
...
public sealed class SetCollisionBits2DNode(
	...
	string nodePath = "") : SpatialActionNodeBase2D(nodePath)
{
	/// <summary>
	...
	{
		if (spatialNode is not CollisionObject2D body)
		{
		...
		CollisionBits2D.Write(
			body,
			_target,
			CollisionBits2D.Apply(CollisionBits2D.Read(body, _target), unchecked((uint)bits), _operation));
	}
}
