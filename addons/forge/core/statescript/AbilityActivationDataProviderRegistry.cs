...
/// <summary>
/// Registry of <see cref="IAbilityActivationDataProvider"/> implementations. All non-abstract providers with a
/// parameterless constructor in the project assembly are discovered automatically, including ones defined in game code,
/// so new activation-data types can be selected on the ability activation nodes without modifying the plugin or editor.
/// </summary>
/// <remarks>
/// Provider instances are cached and shared. Implementations must be stateless:
/// <see cref="IAbilityActivationDataProvider"/> builds a fresh value from the supplied <see cref="GraphContext"/> on
/// every call, so providers should not hold mutable per-activation state.
/// </remarks>
public static class AbilityActivationDataProviderRegistry
{
	private static readonly List<ProviderEntry> _all = [];
	private static readonly Dictionary<string, IAbilityActivationDataProvider> _byIdentifier = [];
	...
	static AbilityActivationDataProviderRegistry()
	{
		foreach (Type type in GetLoadableTypes(Assembly.GetExecutingAssembly()))
		{
			if (type.IsAbstract
				|| type.IsInterface
				|| !typeof(IAbilityActivationDataProvider).IsAssignableFrom(type)
				|| type.GetConstructor(Type.EmptyTypes) is null)
			{
				continue;
			}

			var provider = (IAbilityActivationDataProvider)Activator.CreateInstance(type)!;
	...
	/// <summary>
	/// Describes a discovered activation-data provider for editor display and runtime lookup.
	/// </summary>
	...
	public sealed record ProviderEntry(string Identifier, string DisplayName, IAbilityActivationDataProvider Provider);
}
