...
/// <para>Measured along the path Godot returns, summed corner to corner, in pixels. A destination that cannot be
/// reached is not an error here: the answer is the length of the walk to the closest point the path could get to,
/// which is the honest reading of "how far can I get". Pair it with Nav Reachable 2D when the difference
/// matters.</para>
...
/// a graph branching on distance should ask Nav Reachable 2D first.</para>
/// </remarks>
/// <param name="fromResolver">Resolves where the walk would start.</param>
/// <param name="toResolver">Resolves where it would end.</param>
internal sealed class NavPathLength2DResolver(IPropertyResolver fromResolver, IPropertyResolver toResolver)
	: IPropertyResolver
{
	...
	public Variant128 Resolve(GraphContext graphContext)
	{
		...
		Vector2[] path = NavigationServer2D.MapGetPath(
			...
			new Vector2(toValue.X, toValue.Y),
			true);
