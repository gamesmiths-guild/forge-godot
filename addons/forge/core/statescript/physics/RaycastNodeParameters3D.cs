...
/// <summary>
/// The inputs and outputs the two ray nodes share, so a one-shot cast and a monitored one ask the same question and
/// report it in the same variables.
/// </summary>
internal static class RaycastNodeParameters3D
{
	/// <summary>
		...
		out RaySegment3D segment)
	{
		...
		bool hit = PhysicsQuery3D.TryRaycast(
			world,
			from,
			along,
			(float)maxDistance,
		...
		in RaycastResult3D result)
	{
		WriteVector(
			...
			new NumericsVector3(result.Position.X, result.Position.Y, result.Position.Z));

		WriteVector(
			graphContext,
			outputVariables[HitNormalOutput],
			new NumericsVector3(result.Normal.X, result.Normal.Y, result.Normal.Z));
