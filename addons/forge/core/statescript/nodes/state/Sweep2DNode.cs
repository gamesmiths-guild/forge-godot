...
/// </summary>
/// <remarks>
/// <para>This is Shapecast 2D held open, and it stands to that node exactly as Ray 2D stands to Raycast 2D — the
...
/// <para>The port shape mirrors Condition Monitor and Ray 2D: two edges for the transitions and two subgraphs for the
...
public class Sweep2DNode(bool collideWithAreas = false, bool oneShot = false) : StateNode<Sweep2DNodeContext>
{
	...
	{
		graphContext.GetNodeContext<Sweep2DNodeContext>(NodeID).LastHit = null;
		Cast(graphContext);
	}
	...
	{
		Sweep2DNodeContext nodeContext = graphContext.GetNodeContext<Sweep2DNodeContext>(NodeID);
	...
	private void Cast(GraphContext graphContext)
	{
		...
		else
		{
			...
			PhysicsDebugDraw2D.SetShapecast(
				nodeContext.DebugMarker,
				shape,
				hitTransform,
				segment.From,
