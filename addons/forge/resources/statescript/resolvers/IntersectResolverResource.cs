...
/// <summary>
/// Resolver resource that keeps from a nested array source only the elements also found in a second nested array
/// source. Object-backed elements are matched by reference identity.
/// </summary>
[Tool]
[GlobalClass]
public partial class IntersectResolverResource : ArrayTransformResolverResourceBase
{
	/// <inheritdoc/>
	public override string ResolverTypeId => "Intersect";

	/// <inheritdoc/>
	public override string PropertyNamePrefix => "intersect";

	/// <summary>
	/// Gets or sets the nested resolver providing the elements to keep.
	/// </summary>
	[Export]
		...
		out IObjectArrayResolver? objectArrayResolver)
	{
		...
		if (sourceObjectArray is not null && otherObjectArray is not null)
		{
			...
			{
				GD.PushError(
					"Statescript: Intersect resolver requires matching element types. Got " +
			...
			objectArrayResolver = ArrayResolverResourceUtilities.CreateObjectArrayOperation(
				typeof(ObjectIntersectResolver<>),
				sourceObjectArray,
				otherObjectArray);
			return true;
		}
		...
		if (sourceValueArray is not null && otherValueArray is not null)
		{
			...
			valueArrayResolver = new IntersectResolver(sourceValueArray, otherValueArray);
			return true;
		}

		GD.PushError(
			"Statescript: Intersect resolver cannot mix value-typed and object-backed sources. Using the source " +
			"unchanged.");
		valueArrayResolver = sourceValueArray;
		objectArrayResolver = sourceObjectArray;
		return true;
