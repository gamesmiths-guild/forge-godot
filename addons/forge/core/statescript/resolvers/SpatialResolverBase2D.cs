...
/// <summary>
/// Base for every resolver that reads something off the 2D node an entity lives on.
/// </summary>
/// <remarks>
/// <para>Handles the three things all of them share: resolving the entity operand, getting from that entity to a
/// <see cref="Node2D"/> through <see cref="ForgeEntityBridge"/>, and honoring an authored child path so a marker node
...
internal abstract class SpatialResolverBase2D(IEntityResolver entityResolver, string nodePath) : IPropertyResolver
{
	...
	protected abstract Variant128 ResolveFrom(Node2D spatialNode, GraphContext graphContext);
...
#pragma warning disable SA1202 // Elements should be ordered by access
	...
	public Variant128 Resolve(GraphContext graphContext)
	{
		...
		if (ForgeEntityBridge.TryGetSpatialNode2D(entity, NodePath, out Node2D? spatialNode))
		{
		...
		if (NodePath.Length > 0 && ForgeEntityBridge.TryGetSpatialNode2D(entity, out spatialNode))
		{
			ReportMissingNodeOnce($"found no Node2D at [{NodePath}]. Reading the entity's own node instead.");
			return ResolveFrom(spatialNode, graphContext);
		}

		ReportMissingNodeOnce("found no Node2D for its entity. Resolving to a default value.");
		return default;
	}
