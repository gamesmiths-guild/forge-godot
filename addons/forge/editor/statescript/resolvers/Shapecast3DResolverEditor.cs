...
/// </summary>
[Tool]
internal sealed partial class Shapecast3DResolverEditor : NodeEditorProperty
{
	...
	public override string DisplayName => "Shapecast 3D";
	...
	public override void Setup(
		...
		bool isArray)
	{
		var resource = property?.Resolver as Shapecast3DResolverResource;
		...
		_collideWithAreasCheckBox = new CheckBox
		{
			...
			TooltipText = "Let areas stop the sweep as well as bodies, for entities whose hurtbox is an Area3D.",
		};
		_collideWithAreasCheckBox.Toggled += _ => NotifyChanged();
		root.AddChild(_collideWithAreasCheckBox);

		// Seeded the same way the ray family is: a sphere to sweep, the caster's own position and facing, a reach that
		...
		// upright and an unset one is read as no rotation at all.
	...
	{
		property.Resolver = new Shapecast3DResolverResource
		{
