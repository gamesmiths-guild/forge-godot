	...
	string areaPath = "") : StateNode<Overlap3DNodeContext>
{
		...
		for (int i = 0; i < nodeContext.Changed.Count; i++)
		{
			...
			else
			{
				...
				PhysicsDebugDraw3D.FlashTarget(
					graphContext, changed, PhysicsDebugDraw3D.OverlapFoundColor, _includeAreas);
			}
		...
		IForgeEntity? entity)
	{
		...
		if (_sourceMode == OverlapSourceMode.ExistingArea)
		{
			...
			{
				PhysicsDebugDraw3D.FlashBody(
					graphContext,
					area,
					area.GlobalTransform,
					occupied ? PhysicsDebugDraw3D.OverlapFoundColor : PhysicsDebugDraw3D.OverlapEmptyColor);
			}
