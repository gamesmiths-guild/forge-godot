...
/// </summary>
/// <remarks>
/// <para>Like Character State 3D, these are methods rather than properties, so no expression could reach them before.
...
/// <para><b>Real velocity is the one Entity Velocity 3D cannot give.</b> That resolver reads the velocity the game
...
/// <para>Only a <see cref="CharacterBody3D"/> reports these. Anything else resolves to zero.</para>
/// </remarks>
...
internal sealed class CharacterMotion3DResolver(
	IEntityResolver entityResolver,
	string nodePath,
	CharacterMotionValue value) : SpatialResolverBase3D(entityResolver, nodePath)
{
	...
	protected override Variant128 ResolveFrom(Node3D spatialNode, GraphContext graphContext)
	{
		if (spatialNode is not CharacterBody3D characterBody)
		{
			ReportUnusableNodeOnce(
				...
				" CharacterBody3D does. Resolving to zero.");
		...
		Vector3 result = _value switch
		{
			CharacterMotionValue.FloorNormal =>
				characterBody.IsOnFloor() ? characterBody.GetFloorNormal() : Vector3.Zero,
			CharacterMotionValue.WallNormal =>
				characterBody.IsOnWall() ? characterBody.GetWallNormal() : Vector3.Zero,
