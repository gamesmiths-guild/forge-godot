...
Authors a single `Effect` instance for effect-oriented node inputs in the Godot Statescript editor, combining a `ForgeEffectData` selection with the effect's level and ownership.

## Authoring in Godot

The editor exposes three collapsible sections: **Effect Data**, **Level**, and **Ownership**.

- **Effect Data**: assign one `ForgeEffectData` resource.
